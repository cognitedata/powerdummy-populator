...
def populate_assets(client):
    ...
    transformer_ends = [
        generate_power_transformer_end(name=f"PowerTransformerEnd {i}") for i in range(5)
    ]

    generators = [generate_hydro_generators(name=f"HydroGenerator {i}") for i in range(5)]
    sync_machines = [
        generate_synchronous_machines(name=f"SyncMachine for HydroGenerator {i}") for i in range(5)
    ]

    lines = [generate_ac_line_segment(name=f"ACLineSegment {i}") for i in range(20)]

    al = client.assets.create(
        substations + transformers + generators + sync_machines + transformer_ends + lines
    )
    ...
    term_assets, term_rels = create_terminals(
        substations
    )  # separately to more easily get the terminals to connect
    tl2 = client.assets.create(term_assets)
    tr2 = client.relationships.create(term_rels)
    print(
        f"created {len(tl2)} terminal/analog/sensor assets with {len(tr2)} relationships for substation terminals"
    )
    ...
    for l in lines:
        ...
        rf = generate_relationship(
            subst_terminals[from_ss].external_id, l.external_id, type="connectsTo"
        )
        rt = generate_relationship(
            subst_terminals[to_ss].external_id, l.external_id, type="connectsTo"
        )
        line_rels += [rf, rt]
...
def create_terminals(assets):
    ...
    for asset in assets:
        ...
        rels.append(
            generate_relationship(t.external_id, asset.external_id, "belongsTo")
        )  # terminal connectsTo asset
